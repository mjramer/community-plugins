...
import { createAzureDevopsPullRequestAction } from './devopsPullRequest';
...
describe('publish:azure:pull-request', () => {
  ...
  const action = createAzureDevopsPullRequestAction({ integrations });
  ...
  it('should throw InputError if no credentials or token are provided', async () => {
    const mockContextWithNoCredentials = createMockActionContext({
      input: {
        ...mockContext.input,
        host: 'no-credentials.dev.azure.com',
        token: undefined,
      },
    });

    await expect(action.handler(mockContextWithNoCredentials)).rejects.toThrow(
      InputError,
    );
  });
  ...
  it('should create source branch if it does not exist', async () => {
    mockGitClient.getBranch.mockImplementation((_, branch) => {
      ...
      return { commit: { commitId: 'commit-id' } };
    });
    mockGitClient.createPush.mockResolvedValue({});
